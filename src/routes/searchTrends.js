...
Now install the google-trends-api package:
bashnpm install google-trends-api
Alternative: Using ScrapingDog with a Trends Aggregator
If you want to use ScrapingDog exclusively, you could scrape a trends aggregator site instead:
javascript// Alternative approach using ScrapingDog to scrape trends data
router.get('/brand/:brandName/scraping', async (req, res) => {
  try {
    const { brandName } = req.params;
    
    // Use a trends aggregator that's easier to scrape
    // For example, scraping Google Shopping insights or similar
    const searchUrl = `https://shopping.google.com/search?q=${encodeURIComponent(brandName)}&gl=GB`;
    ...
    // You would need to parse the HTML here
