    ...
    for (const doc of customersSnapshot.docs) {
      try {
        ...
        if (exists) {
          ...
          updates['financial.outstanding_amount'] = parseFloat(zoho.outstanding_receivable_amount || oldData.outstanding_receivable_amount || 0);
          updates['financial.overdue_amount'] = parseFloat(zoho.overdue_amount || 0);
          updates['financial.unused_credits'] = parseFloat(zoho.unused_credits_receivable_amount || 0);
          
          // Update metrics
          updates['metrics.total_spent'] = parseFloat(totalSpent);
          updates['metrics.total_invoiced'] = parseFloat(zoho.total_invoiced || totalSpent);
          updates['metrics.total_paid'] = parseFloat(totalSpent) - parseFloat(zoho.outstanding_receivable_amount || 0);
          updates['metrics.payment_performance'] = parseFloat(zoho.payment_performance || 100);
          updates['metrics.average_order_value'] = parseFloat(oldData.average_order_value || firebase.average_order_value || 0);
          updates['metrics.order_count'] = parseInt(orderCount);
          updates['metrics.invoice_count'] = parseInt(zoho.invoice_count || orderCount);
          
          // Only add last_order_date if it exists
          if (firebase.last_order_date || oldData.last_order_date) {
            updates['metrics.last_order_date'] = firebase.last_order_date || oldData.last_order_date;
          }
          
          // Only add days_since_last_order if we have a last_order_date
          if (firebase.last_order_date) {
            updates['metrics.days_since_last_order'] = Math.floor((Date.now() - new Date(firebase.last_order_date).getTime()) / (1000 * 60 * 60 * 24));
          }
          
          // Update enrichment data if available
          updates['enrichment.segment'] = segment;
          
          // Update sync information
          updates['sync.last_synced'] = oldData._lastSynced || oldData.last_synced || admin.firestore.Timestamp.now();
          updates['sync.sync_status'] = oldData.sync_status || 'success';
          
          // Only add zoho_last_modified if it exists
          if (zoho.last_modified_time) {
            updates['sync.zoho_last_modified'] = zoho.last_modified_time;
          }
          
          // Update modified date
          updates['last_modified'] = admin.firestore.Timestamp.now();
          updates['updated_by'] = 'migration_script';
          
          // Add migration metadata
          updates['_migration.last_updated'] = admin.firestore.Timestamp.now();
          updates['_migration.update_count'] = admin.firestore.FieldValue.increment(1);
          ...
          const newCustomer = {
            ...
            ...(oldData.customer_id || zoho.contact_id || zoho.customer_id ? { zoho_customer_id: oldData.customer_id || zoho.contact_id || zoho.customer_id } : {}),
            ...(oldData.firebase_uid ? { firebase_uid: oldData.firebase_uid } : {}),
            ...
            metrics: (() => {
              ...
              // Only add first_order_date if it exists
              if (firebase.first_order_date || zoho.created_time) {
                metrics.first_order_date = firebase.first_order_date || zoho.created_time;
              }
              ...
              // Only add days_since_last_order if we have a last_order_date
              if (firebase.last_order_date) {
                metrics.days_since_last_order = Math.floor((Date.now() - new Date(firebase.last_order_date).getTime()) / (1000 * 60 * 60 * 24));
              }
              
              // Only add customer_lifetime_days if we have created_time
              if (zoho.created_time) {
                metrics.customer_lifetime_days = Math.floor((Date.now() - new Date(zoho.created_time).getTime()) / (1000 * 60 * 60 * 24));
              }
            ...
            sync: (() => {
              ...
              // Only add zoho_last_modified if it exists
              if (zoho.last_modified_time) {
                sync.zoho_last_modified = zoho.last_modified_time;
              }
